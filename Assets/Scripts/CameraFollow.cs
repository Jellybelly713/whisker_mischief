...
public class CameraFollow : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Vector3 desiredPosition = player.position + player.forward * -10f + Vector3.up * 10f;
        transform.position = Vector3.SmoothDamp(transform.position, desiredPosition, ref currentVelocity, smoothTime);

        //if the player rotates so does the camera ----- player look left camera look left
        Quaternion desiredRotation = Quaternion.LookRotation(player.position - transform.position) * tiltOffset;
        ...
        transform.RotateAround(player.position, Vector3.up, Input.GetAxis("Horizontal") * rotationSpeed * Time.deltaTime);
